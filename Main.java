        ...
        do {
                ...
                do {
                    ...
                    try {
                        ...
                        do {
                            ...
                            if (eta < etaMinima) {
                                System.out.println("Valore non valido, l'età minima è di " + etaMinima + " anni");
                            }
                    ...
                    try {
                        ...
                        do { //verifica che l'età inserita sia valida
                            ...
                            if (eta < ETA_MIN_GIUDICI) {
                                System.out.println("Valore non valido, l'età minima è di " + ETA_MIN_GIUDICI + " anni");
                            }
            ...
            for (int j = 0; j < partecipantiTotali; j++) {  //per ogni partecipante
                ...
                do {    //verifica che i voti inseriti siano validi
                    ...
                    // Il giudice corrente assegna i voti per le tre prove al partecipante j
                    // I voti vengono memorizzati in liste separate all'interno dell'oggetto partecipante
                    giudice.get(i).assegnaVoti(votoPratica, votoScritto, votoOrale , partecipanti.get(j));   //assegna i voti al partecipante
            ...
            for (Partecipante p : partecipanti) {
                ...
                // Somma tutti i voti assegnati da ciascun giudice per ogni tipo di prova
                ...
                // Memorizza le medie calcolate nell'oggetto partecipante
            ...
            do {    //verifica che la scelta inserita sia valida
                ...
                if (visualizzaClassifica.equals("si")) {
                    ...
                    for (int i = 0; i<partecipantiTotali; i++) {
                        //System.out.println("Partecipante " + (i+1) + ": " + partecipanti.get(i).getMediaGenerale());    //si può anche stampare solo la media generale
                        partecipanti.get(i).visualizza(); //stampa i dati del partecipante
                    }
            ...
            do {
                System.out.println("Vuoi fare un altro concorso? (si/no)");
                continua = scanner.nextLine().toLowerCase(); 
                //verifica che la scelta inserita sia valida
                if (!continua.equals("si") && !continua.equals("no")) {
                    System.out.println("Scelta non valida, riprova.");
                } else {
                    partecipanti.clear(); //svuota la lista dei partecipanti
                    giudice.clear(); //svuota la lista dei giudici
                }
            } while (!continua.equals("si") && !continua.equals("no")); //verifica che la scelta inserita sia valida
        
        } while (continua.equals("si")); //se la scelta è si, continua il ciclo
        scanner.close();
